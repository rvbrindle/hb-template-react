...
export default function NavBar(
    ...
    return (
        <>
            {/* Desktop Nav */}
            <nav
                className={`w-full bg-${primary} text-${textColor} py-2 z-50 border-b-2 border-${tertiary} ${setHeaderFixedOnScroll()}`}>
                <div className='container mx-auto flex flex-row justify-between'>
                    ...
                    >
                        <div
                            id='burger-bar-1'
                            className={`w-1/2 bg-${tertiary} h-1 origin-center transition-all ease-in-out duration-500`}></div>
                        <div
                            id='burger-bar-2'
                            className={`w-1/2 bg-${tertiary} h-1 my-2 sm:my-4 transition-all ease-in duration-200`}></div>
                        <div
                            id='burger-bar-3'
                            className={`w-1/2 bg-${tertiary} h-1 origin-center transition-all ease-in-out duration-500`}></div>
                    </div>
            ...
            <nav className={`w-full flex justify-end ${setHeaderFixedOnScroll()} relative`}>
                <ul
                    onClick={() => toggleMenu()}
                    id='nav-menu-mobile'
                    className={`hidden md:hidden flex-col items-center w-3/4 bg-${secondary} z-50 py-3 fixed`}>
                    {navItems}
                </ul>
            </nav>
        </>
